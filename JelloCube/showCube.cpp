...
void showInclinedPlane()
{
	int i, ip;
	double length, d;
	point p1, p2, normal, tangent, bitangent, v[4];
	...
	p1 = { -jello.d / jello.a, 0, 0 };
	p2 = { 0, -jello.d / jello.b, 0 };
	pDIFFERENCE(p1, p2, tangent);
	pNORMALIZE(tangent);

	CROSSPRODUCTp(tangent, normal, bitangent);

	//glColor4f(0.9, 0.6, 0.6, 0);
	glColor4f(0.6f, 0.7f, 0.6f, 0.5f);

	// plane vertices to draw
	v[0] = { p1.x - tangent.x * d - bitangent.x * d, p1.y - tangent.y * d - bitangent.y * d, p1.z - tangent.z * d - bitangent.z * d };
	v[1] = { p1.x + tangent.x * d - bitangent.x * d, p1.y + tangent.y * d - bitangent.y * d, p1.z + tangent.z * d - bitangent.z * d };
	v[2] = { p1.x + tangent.x * d + bitangent.x * d, p1.y + tangent.y * d + bitangent.y * d, p1.z + tangent.z * d + bitangent.z * d };
	v[3] = { p1.x - tangent.x * d + bitangent.x * d, p1.y - tangent.y * d + bitangent.y * d, p1.z - tangent.z * d + bitangent.z * d };
	...
	if (viewingMode == 0)
	{
		...
		for (i = 0; i <= 3; i++)
		{
			...
			glVertex3f(v[ip].x, v[ip].y, v[ip].z);
		}
	}
	else
	{
